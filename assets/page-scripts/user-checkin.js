...
createApp({
  setup() {
    ...
    // testMode: true เมื่อ URL มี ?test=1 (ติ๊ก checkbox Test ในหน้า QR Code)
    ...
    // ตรวจว่า error ที่ได้เป็น "access token revoked" หรือไม่
    ...
    // ล้าง LIFF session และ localStorage ที่เกี่ยวข้องกับ LIFF token
    function clearLiffSession() {
      try {
        // ลบ key ที่ LIFF SDK ใช้เก็บ token
        const keysToRemove = [];
    ...
    async function initLiff() {
      try {
        ...
        // ปิด withLoginOnExternalBrowser: false เพื่อให้โค้ดเราควบคุม redirectUri เองทุกครั้ง
        await withTimeout(
          liff.init({
        ...
        if (liff.isLoggedIn()) {
          ...
          try {
            ...
            if (isTokenRevokedError(profileErr)) {
              // Token ถูก revoke หรือหมดอายุ — ล้าง session แล้ว login ใหม่อัตโนมัติ ไม่ต้องสแกน QR ใหม่
              ...
              // getProfile ล้มเหลวด้วยเหตุอื่น (network ชั่วคราว) — ใช้ profile เป็น null แต่ยังให้เช็คอินได้
              authState.value = "logged_in";
              setStatus("idle", "พร้อมเช็คอิน", "โหลดข้อมูลบางส่วนไม่สำเร็จ แต่ยังเช็คอินได้");
            }
          }
        } else {
          authState.value = "logged_out";
        ...
        // ตรวจว่าเป็น token revoked หรือไม่ ถ้าใช่ ให้ล้าง session และ login ใหม่อัตโนมัติ
        if (isTokenRevokedError(err)) {
          ...
          // LIFF error ทั่วไป (network, timeout, SDK ไม่โหลด) — ให้กดปุ่ม retry ได้
    ...
    // testMode = true: เก็บ 10 ครั้ง ทุก 1 วินาที บันทึกทันทีทุกครั้ง ไม่ตรวจ Geofence ลง Sheet "Test"
    // testMode = false: เก็บ 5 ครั้ง ทุก 2 วินาที เลือก best ตรวจ Geofence ลง Sheet "Logs" (เดิม)
    async function startGPSSampling() {
      ...
      // ⚠️ FIX เพิ่มเติม: เดิมค่าแรก (1/5) จะถูกรับ "ทันที" ที่มือถือหาสัญญาณ GPS เจอ (ไม่รอ 2 วิ)
      // ทำให้เวลารวมคุมไม่ได้ เพราะขึ้นกับว่ามือถือใช้เวลาหาสัญญาณครั้งแรกนานแค่ไหน (ไม่แน่นอน)
      // แก้โดยเริ่มนับ lastAcceptedAt จาก "เวลาที่กดปุ่ม" (samplingStartTime) แทนค่า 0
      // และเอาเงื่อนไข readingsCollected > 0 ออก เพื่อให้ค่าแรกก็ต้องรอครบ 2 วิเหมือนค่าที่เหลือ
      // ผลคือทุกค่ารวมถึงค่าแรกจะห่างกันแน่นอน ~2 วิ ตั้งแต่กดปุ่ม รวม 5 ค่า ~10 วิ ที่คาดเดาได้
      ...
      watchId = navigator.geolocation.watchPosition(
        (pos) => {
          ...
          if (readingsCollected < MAX_READINGS) {
            ...
            setStatus(
              "loading",
              `กำลังเก็บข้อมูล GPS${testMode ? " [TEST]" : ""} (${readingsCollected}/${MAX_READINGS})...`,
              `ล่าสุด: ละติจูด ${lat.toFixed(6)}, ลองจิจูด ${lng.toFixed(6)}, ความแม่นยำ ${accuracy.toFixed(1)} เมตร`,
            );

            // ============================================================
            // GPS TEST MODE: บันทึกทันทีทุกครั้งที่อ่านค่าได้ ไม่ตรวจ Geofence
            ...
            if (readingsCollected >= MAX_READINGS) {
              ...
              if (testMode) {
                // TEST MODE: แจ้งสำเร็จ ไม่ redirect ไปหน้า processing
                setStatus(
                  "success",
