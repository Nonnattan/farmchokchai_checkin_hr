      ...
      createApp({
        setup() {
          ...
          function focusArea(area) {
            ...
            areaRect = L.rectangle([[b.minLat, b.minLng], [b.maxLat, b.maxLng]], {
              color: "#2563eb",
              weight: 2,
              fillColor: "#2563eb",
              fillOpacity: 0.10
            ...
            selectedMarker = L.marker([b.centerLat, b.centerLng]).addTo(map).bindPopup(`<b>${area.areaName}</b><br/>${area.areaId}`);
            selectedMarker.openPopup();
            map.flyTo([b.centerLat, b.centerLng], 17, { animate: true, duration: 0.6 });
          ...
          async function loadData() {
            ...
            try {
              ...
              await nextTick();
              initMap();
              if (areas.value.length) focusArea(areas.value[0]);
            } catch (err) {
          ...
          onMounted(async () => {
            try {
              ...
              if (!map) {
                await nextTick();
                initMap();
                if (selectedArea.value) focusArea(selectedArea.value);
              }
            } catch (err) {
          ...
          return {
            session, areas, loading, statusType, statusTitle, statusDesc,
            selectedId, selectedArea, navItems, mapEl, focusArea, renderTableFocus, logout, nav, common
          };
        },
        template: `
          <div class="container">
              ...
              <section class="card stack">
                ...
                <div class="table-wrap">
                  <table class="table">
                    ...
                    <tbody>
                      <tr v-for="a in areas" :key="a.areaId" class="click-row" @click="focusArea(a)">
                        <td>
                          <b>{{ a.areaName }}</b><br/>
                        ...
                        <td>
                          <span class="pill-sm" :style="{background:a.active===false ? '#fef2f2' : '#ecfdf3', color:a.active===false ? '#b91c1c' : '#047857'}">
                            {{ a.active===false ? 'inactive' : 'active' }}
                          </span>
                        </td>
                        <td>
                          <div class="actions">
                            <a class="btn ghost" :href="'./qr_code.html?areaId=' + encodeURIComponent(a.areaId)" @click.stop>ดู QR</a>
                            <a v-if="session?.role === 'masteradmin'" class="btn primary" href="./setarea.html" @click.stop>SetArea</a>
                          </div>
                        </td>
                      </tr>
                    </tbody>
                ...
                <div class="mini-note">Masteradmin เห็นทุกอย่างและเข้า SetArea/Users ได้ ส่วน admin เห็นหน้าตารางกับแผนที่เหมือนกัน แต่ปุ่ม SetArea จะไม่แสดง</div>
              </section>
